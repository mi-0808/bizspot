      ...
      <main className="app-shell px-3 pb-[calc(116px+env(safe-area-inset-bottom))] pt-[max(env(safe-area-inset-top),18px)]">
        ...
        <div className="surface-card fixed inset-x-0 bottom-3 z-30 mx-auto flex w-[calc(100%-24px)] max-w-[406px] items-center justify-between rounded-[28px] px-4 py-3">
          <button
            ...
            className={`flex flex-1 items-center justify-center gap-2 rounded-[22px] px-3 py-3 text-sm font-semibold ${
              viewMode === "map" ? "bg-slate-950 text-white" : "text-slate-500"
            }`}
          >
          ...
          <button
            type="button"
            onClick={() => setFilterSheetOpen(true)}
            className="mx-2 rounded-full bg-[linear-gradient(180deg,#3b82f6_0%,#2563eb_100%)] p-4 text-white shadow-[0_12px_24px_rgba(37,99,235,0.3)]"
            aria-label="条件検索"
          >
          ...
          <button
            ...
            className={`flex flex-1 items-center justify-center gap-2 rounded-[22px] px-3 py-3 text-sm font-semibold ${
              viewMode === "list" ? "bg-slate-950 text-white" : "text-slate-500"
            }`}
          >
